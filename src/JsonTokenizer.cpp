...
namespace JStream {
    ...
    JsonTokenizer::Token JsonTokenizer::next(String* buf) {
        ...
        while(is->hasNext()) {
            ...
            if(JStream::isDecDigit(is->peek()) || is->peek() == '-') {
    ...
    void JsonTokenizer::skipWhitespace() const {
        while(is->hasNext() && JStream::isWhitespace(is->peek())) {
            is->next();
        }
    }
        ...
        **/
        ...
        while(is->hasNext()) {
            ...
            if(state == FSM_State::INT || state == FSM_State::DECIMAL || state == FSM_State::EXP) {
                while(is->hasNext() && JStream::isDecDigit(is->peek())) {
                    char c = is->next();
                    if(buf != nullptr) *buf += c;
                }
            } else {
        ...
        if(is->hasNext() && JStream::isNumDigit(is->peek())) {
            errorCode = ParseError::MALFORMED_NUM;
            while(is->hasNext() && JStream::isNumDigit(is->peek())) {
                char c = is->next();
                if(buf != nullptr) *buf += c;  
            }
            return false;
    ...
    bool JsonTokenizer::readStr(String* buf) {
        while(is->hasNext()) {
            if(is->peek() == '\\') {
                ...
                else if(JStream::isEscapeable(is->peek())) {
                    if(buf != nullptr) *buf += JStream::escape(is->next());
                    else is->next();
                } else {
